...
public class JLoggReader extends Reader {
	...
	/**
	 * MODIFIED EVERYTHING BELOW THIS IS EITHER MODIFIED CODE OR NEWLY INTRODUCED
	 * CODE
	 */

	/**
	 * Static method to open an lazy stream of JLoggReaderLines. (Heavily inspired
	 * by Files.lines(...))
	 * 
	 * @param path
	 * @param cs
	 ...
	 * @throws IOException
	 */
	public static Stream<JLoggReaderLine> lines(Path path, Charset cs) throws IOException {
		JLoggReader br = new JLoggReader(path, cs);
		try {
	...
	/**
	 * Made this constructor private => only path constructo should be used Creates
	 * a buffering character-input stream that uses an input buffer of the specified
	 * size.
	 *
	 *
	 * @param in A Reader
	...
	/**
	 * Main constructor to be used from calling code
	 * 
	 * @param path
	 * @param cs
	 * @throws IOException
	 */
	private JLoggReader(Path path, Charset cs) throws IOException {
		this(new InputStreamReader(path.getFileSystem().provider().newInputStream(path), cs.newDecoder()));
	}

	/**
	 * Read a line from the file, make sure to keep track of the amount of delimiter
	 * characters used within the line
	 * 
	 * @param ignoreLF
	 * @return
	 * @throws IOException
	 */
	private JLoggReaderLine readLine() throws IOException {
		StringBuffer s = null;
		int startChar;
			...
			for (;;) {
				...
				if (nextChar >= nChars) { /* EOF */
					if (s != null && s.length() > 0) {
						return new JLoggReaderLine(s.toString(), amountOfDelim);
					} else {
						return null;
					}
				}
				...
				if (eol) {
					...
					if (c == '\r') {
						if (nextChar < cb.length) {
							...
							// this basically means that we have to refill + try again
							fill();
							if (nextChar < cb.length && cb[nextChar] == '\n') {
								amountOfDelim = 2;
							}
						}
						skipLF = true;
					}
					return new JLoggReaderLine(str, amountOfDelim);
				}
	...
	public Stream<JLoggReaderLine> lines() {
		Iterator<JLoggReaderLine> iter = new Iterator<JLoggReaderLine>() {
			JLoggReaderLine nextLine = null;
			...
			@Override
			public JLoggReaderLine next() {
				if (nextLine != null || hasNext()) {
					JLoggReaderLine line = nextLine;
					nextLine = null;
					return line;
				} else {
